...
async function prop(scene: THREE.Scene, name: string, x: number, z: number, height: number, ry = 0, colliders?: Collider[]): Promise<void> {
...
export async function buildAmbient(scene: THREE.Scene, colliders: Collider[] = []): Promise<void> {
  ...
  await prop(scene, "campfire", -40, 28, 1.2, 0, colliders);
  ...
  await prop(scene, "route-marker", 18, -22, 1.6, 0, colliders);     // beside the road to the crossing
  await prop(scene, "route-marker", 51, 4, 1.6, 0, colliders);       // beside the road to Isengard
}
